...
export default function ExplanationPopup({ isOpen, onClose, onSubmit, initialExplanation }: ExplanationPopupProps) {
  ...
  return (
    <div className="fixed top-24 right-8 z-[100] flex flex-col w-[400px] pointer-events-none">
      {/* Panel Container */}
      <div 
        className="relative bg-white/70 backdrop-blur-2xl w-full rounded-3xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1),0_0_0_1px_rgba(0,0,0,0.05)] border border-white/50 flex flex-col overflow-hidden transition-all duration-300 ease-[cubic-bezier(0.23,1,0.32,1)] opacity-100 scale-100 ring-1 ring-black/5 pointer-events-auto h-[420px]"
        style={{
      ...
      >
        ...
        <div className="p-4 flex-grow flex flex-col min-h-0">
          <label className="block text-[10px] font-bold text-slate-500 mb-2 uppercase tracking-widest text-center">Nhập lời giải thích</label>
          <div className="relative rounded-xl overflow-y-auto border border-slate-200 shadow-inner bg-slate-50/80 flex-1 flex flex-col">
            <RichTextEditor 
              content={explanation}
        ...
        {/* Footer */}
        <div className="px-4 py-3 bg-slate-50/80 backdrop-blur-md flex items-center justify-between border-t border-slate-100 shrink-0">
          <button 
            onClick={handleClose}
            className="px-4 py-2 rounded-xl font-bold text-slate-600 bg-white/50 hover:bg-slate-100 transition-all text-sm flex-1 mr-2"
          >
            Hủy
          </button>
          <button 
