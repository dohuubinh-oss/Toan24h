...
export default function AIHintPanel({ isOpen, onClose, question, unlockedLevel, onUnlock, readonlyMode = false }: AiHintPanelProps) {
  ...
  return (
    <aside 
      ref={panelRef}
      className={`absolute top-0 bottom-0 right-0 w-[500px] bg-white  border-l border-slate-200  shadow-2xl transition-transform duration-500 z-[10000] flex flex-col
    ...
    >
      <div className="p-6 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h3 className="font-bold text-slate-800">Gợi ý</h3>
        </div>
        <button 
          onClick={onClose}
          className="p-2 hover:bg-slate-200 rounded-lg cursor-pointer text-slate-500 transition-colors"
        >
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        {hints.map((hint) => {
          const isUnlocked = readonlyMode || (hint.level <= unlockedLevel)
          const isNextToUnlock = !readonlyMode && (hint.level === unlockedLevel + 1)
          ...
          if (hint.color === 'red') {
            colors = { bg: 'bg-red-50 ', border: 'border-red-100 ', icon: 'text-red-600 ', title: 'text-red-900 ', text: 'text-red-800 ' }
          } else if (hint.color === 'amber') {
            colors = { bg: 'bg-amber-50 ', border: 'border-amber-100 ', icon: 'text-amber-600 ', title: 'text-amber-900 ', text: 'text-amber-800 ' }
          } else if (hint.color === 'indigo') {
            colors = { bg: 'bg-indigo-50 ', border: 'border-indigo-100 ', icon: 'text-indigo-600 ', title: 'text-indigo-900 ', text: 'text-indigo-800 ' }
          }
            ...
            return (
              ...
              >
                <div className="absolute inset-0 bg-slate-900/60 hover:bg-slate-900/70 backdrop-blur-[2px] z-10 flex items-center justify-center flex-col gap-2 transition-all cursor-pointer">
                  <div className="p-3 bg-white/90 rounded-full shadow-lg text-primary group-hover:scale-110 transition-transform">
                ...
                <div className="bg-slate-50 p-4 border border-slate-200 blur-[3px] select-none">
        ...
        {question.solution_guide && (
          (readonlyMode || unlockedLevel >= hints.length + 1) ? (
            <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-200">
              <div className="flex items-start gap-3">
                <GraduationCap className="w-5 h-5 text-emerald-600 mt-0.5 shrink-0" />
                <div className="flex-1 overflow-hidden">
                  <h4 className="font-semibold text-emerald-900 text-sm mb-2">Giải chi tiết</h4>
                  <div className="text-sm text-emerald-800 leading-relaxed max-w-full overflow-x-auto">
            ...
            unlockedLevel >= hints.length && (
              ...
              >
                <div className="absolute inset-0 bg-slate-900/70 hover:bg-slate-900/80 backdrop-blur-[4px] z-10 flex items-center justify-center flex-col gap-2 transition-all cursor-pointer">
                  <div className="p-3 bg-amber-400 rounded-full shadow-lg text-white group-hover:scale-110 transition-transform">
                ...
                <div className="bg-emerald-50 p-6 border border-emerald-200 blur-[4px] select-none">
                  <div className="w-full h-6 bg-slate-300 rounded mb-3"></div>
                  <div className="w-11/12 h-6 bg-slate-300 rounded mb-3"></div>
                  <div className="w-4/5 h-6 bg-slate-300 rounded"></div>
                </div>
              </button>
            )
          )
